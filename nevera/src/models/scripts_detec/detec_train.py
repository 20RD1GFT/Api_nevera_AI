...
# generate tfrecords
os.system("python {record_script} -x {os.path.join(IMAGE_PATH, 'train')} -l {f'{annotations_path}/label_map.pbtxt'} -o {os.path.join(annotations_path, 'train.record')}")
os.system("python {record_script} -x {os.path.join(IMAGE_PATH, 'test')} -l {f'{annotations_path}/label_map.pbtxt'} -o {os.path.join(annotations_path, 'test.record')}")

TRAINING_SCRIPT = os.path.join(api_path, 'research', 'object_detection', 'model_main_tf2.py')

command = "python {} --model_dir={} --pipeline_config_path={} --num_train_steps=1".format(TRAINING_SCRIPT, model_path, pipeline_file)
